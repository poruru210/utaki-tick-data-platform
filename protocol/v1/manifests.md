...
全てのM3 derivative keyは次のscope-relative baseを使います。
...
Protocol helperがrelative keyを検証して導出し、trusted `r2.Layout`はそのrelative keyへimmutable rootとscope prefixを一度だけprependします。

relative keyはscope-relativeであるため、dataset、source、symbolのscope bindingはcanonical manifestとtrusted `r2.Layout`のscope prefixで閉じます。

relative keyだけを別scopeへ移すことはfull remote keyの同一性を満たさず、Layout verifierが拒否します。

relative physical keyの最大長は1024 bytesです。

## raw-day-manifest-v1

**`raw-day-manifest-v1`**：一つのdataset、source、symbol、日付に対応するraw保存範囲を記録するmanifestです。
...
raw WAL objectのkeyはscope-relativeなASCII文字列`objects/raw/wal-<64 lowercase sha256>.rtw`であり、`sha256`から機械的に導出します。
...
scope chainがsegment間で不連続な場合はmanifestをrejectします。

objects配列はscope chainの順序で並べます。
...
raw manifest keyは、scope prefixを除くscope-relativeな`raw-day-<revision>-<domain digest>.json`へ機械的に導出し、keyとdomain digestを同じmanifestへbindします。

replay source APIの`ManifestRelativeKey`は、このscope-relative keyとの完全一致だけを受理します。
...
`dataset_id`、`day_definition_id`、`date`、`replay_contract_id`、`conversion_id`、`raw_day_manifest_key`、`raw_day_manifest_sha256`は、partの正確なReplayScopeとraw-day manifest revisionをbindします。
...
| raw_day_manifest_key | string | exact scope-relative raw manifest key |
...
同じpart set内の全partはdataset、day definition、date、replay contract、raw manifest key+digest、ConversionTupleが完全一致しなければなりません。
...
Protocol V1 helperがscope-relative keyを導出し、trusted `r2.Layout`がimmutable root、dataset、provider、feed、symbol、scope prefixを一度だけprependします。
