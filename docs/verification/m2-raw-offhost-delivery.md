...
この記録は2026-07-15時点のM2R-4実装とローカル検証境界を記録します。
...
この記録でCI workflowの実行成功や実R2 smokeの成功を主張しません。
...
empty-cache fetch、実production APIを使うpublicationからreaderまでの接続、exact BatchFrameV1 bytes、zero-record error batch、day reportの`anchored_day_slice`、campaign reportの`genesis_to_root`は`TestM2RawOffhostDeliveryEndToEndFake`で確認します。
...
GitHub Actionsのcheck workflowとWindows race workflowは追加しましたが、この作業環境からworkflowの成功を確認していません。
