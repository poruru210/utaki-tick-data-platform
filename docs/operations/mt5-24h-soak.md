...
- MQL ServiceとGatewayが1対1で、Gatewayはloopback listener、WAL、journal、outbox、lockを
  他scopeと共有しない。
- source、Gateway、R2/rclone、disk、terminal healthのbaselineがhealthyである。
...
| 10:00 | rclone timeout/retry注入 | immutable same-content retry、different bytes拒否 |
...
| 18:00 | publisher handover | old stop/revoke、new epoch、conditional transition |
...
disk free、goroutine/heap、R2 request failure、rclone retry、API timeout、recovery timeを
...
- R2またはrclone障害から復旧後に、source errorとgapの境界が説明できない
