...
real R2 smokeは、隔離bucketまたはprefix、明示的credential、pinned rclone binary、非上書き確認が揃った場合だけ実行し、条件がなければskipします。
...
R2のsealerはfull keyとrclone keyをtrusted `r2.Layout`だけから導出し、local path、receipt path、rclone binary pathをcanonical identityへ含めません。
...
Receiptはcomplete canonical bundle、terminal final observation、bundle／observation digest、claim、roots、limits、rclone identityを再検証し、journal、stage、event、retry、ETag、time、secret、local pathを含みません。
...
R7初回監査は、expected descriptor bytesのI/O前課金、terminal二回目readのclassification collapse、Parquet rclone checkのactual-byte不明をresource bypassとして指摘し、R3をG3Eで再openしました。
...
G9Eはdirect helperを削除し、production `r2.NewPublisher`と`Publisher.Publish`をnetwork-free fake backend、temporary `PublicationJournal`、capability限定fake rcloneへ接続しました。
...
Fake rcloneは`version`、`copyto --immutable`、`check --download`だけを受理し、任意operation、flag差し替え、trusted prefix外keyを拒否します。
