...
M2R-2はAWS SDK for Go v2のR2 publication boundary、scope publisher claim、local exclusive lock、独立publication journal、immutable transfer、remote recheck、verification receiptを提供します。

M2R-3はread-only ArchiveReader、tickctlのdatasets、scopes、snapshots raw、fetch、tick-verifyのday、scope commandsを提供します。
...
通常のM2検証はfake backendだけで完結し、R2 smokeはisolated bucket、`smoke/` immutable root、endpoint、credentialを要求します。
...
bounded backend、trusted `r2.Layout`、scope／epoch lock、pinned tool allow-list、local no-clobber primitive、旧testが表すuseful fault caseは境界を狭めて再利用します。
...
R2 smokeは、隔離bucket、`smoke/` immutable root、明示的credentialが揃った場合だけ実行し、条件がなければskipします。
...
Aggregate budget enforcementはscope全体で共有し、receiptがbindするfinal observation counterは最後のfresh passだけを表します。

Lock取得後かつremote action前の全local derivative再検証を復元し、G5 legacy removalをunblockしました。

G5はseal／preflight、publication lock、fresh observe、pure reconcile、approved ObjectID一件のexecute、fresh reobserve、canonical receipt no-clobberだけを接続するthin publisherを完成しました。
...
Replay day reportはraw binding／semantics、part chain／root、Parquet schema／rows／file hash、row-chain rootを分離し、empty dayを受理しながらscope genesis全体を検証したとは主張しません。
...
Replay publisherのlock identityは任意filenameではなく、lock rootとtrusted Layoutのscope／publisher epochから導出するcanonical pathへ固定しました。
