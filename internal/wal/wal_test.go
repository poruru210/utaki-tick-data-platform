...
func TestWALAppendsAndRecoversIncompleteTail(t *testing.T) {
	...
	recovered, err := testsupport.NewStartedWAL(root, "gateway-test-01")
	if err != nil {
		t.Fatal(err)
	}
...
func TestWALOpensFromPruneAnchor(t *testing.T) {
	...
	first, err := testsupport.NewStartedWAL(root, "gateway-test-01")
	if err != nil {
		t.Fatal(err)
	}
	...
	reopened, err := testsupport.NewStartedWALWithAnchor(root, "gateway-test-01", anchor)
	if err != nil {
		t.Fatal(err)
	}
