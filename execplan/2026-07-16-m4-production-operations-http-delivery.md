...
M4-0は本計画だけを固定し、pruning、HTTP service、remote write、実機soakを開始または完了したとは扱わない。
...
credential変更はGatewayを停止した市場休場中に行い、実行中のGatewayで認証情報や公開担当を切り替えない。
...
24時間soakではprocess restart、MT5 restart、forced reboot、networkまたはR2停止、R2 SDK timeoutとretry、disk high-water、Gateway長時間停止を注入する。
...
- [x] (2026-07-16、M4-0 plan freeze) M4をM4-1からM4-9の順序付きgateへ分け、proof-gated pruning、disk pressure、multi-scope運用、HTTP adapter、fault/load、外部環境、final auditの停止条件を固定した。
- [x] (2026-07-16、M4-1完了) retention proof、local prune checkpoint、HTTP responseのversioned contractとresource limitを固定した。Go/Python strict decoder、focused test、fixture verifier、ruff、diff checkを通過し、GPT-5.5 xhigh self-reviewをcleanで完了した。
...
- [x] (2026-07-17、M4-4整理) 市場休場中の停止・設定更新・再起動で対応する方針を採用し、未使用のオンライン認証情報切替機能と専用smokeを実装対象から削除した。
- [x] (2026-07-17、M4-4再レビュー) GPT-5.5 xhigh workerでオンライン認証情報切替機能削除後の差分を再レビューし、初回指摘のPublicationJournal再起動状態、forced reboot手順の現行API記述、ExecPlanの歴史記述境界を修正した。修正後の対象Goテストとworker再レビューはPASS（actionable findingなし）となった。
...
- [x] (2026-07-16、M4-7 GitHub CI verification) PR #5のcommit `1ebecf724fb588eaeff556e664f6bb92e331eae2`について、pushイベントとpull_requestイベントのRepository check、Linux Race、Windows Raceを実行し、6実行すべてをsuccessで確認した。push runはRepository `29464847156`、Linux Race `29464847151`、Windows Race `29464847154`、PR runはRepository `29464861491`、Linux Race `29464861398`、Windows Race `29464861412`である。Race artifact uploadとtoolchain metadata captureも各workflowでsuccessだった。artifactの保存期限はworkflow設定どおり14日であり、長期の外部証拠保存はM4-9で別途確認する。
- [ ] M4-8で隔離real R2 smokeと、1 broker・1 symbolの24時間以上の実機soakを完了する。raw smoke、credential変更手順、別read-only credential、MT5実機証跡は未実施。
...
- 観察: M2 raw publisherはSQLite stageを再開情報として使い、M3 replay publisherはsealed bundleとfresh observationをauthorityにする。
  判断: pruningは既存stageの順位をauthorityへ流用せず、検証済みinputとfresh observationから毎回planを再計算する。
- 観察: 現行ingest configは一つのproducer identity、scope、exact source symbol、listen address、WAL root、journal pathを持つ。
...
- 観察: forced reboot、terminal history retention、credential変更、real R2 outageはunit testだけでは証明できない。
  判断: M4-7のlocal gateがpassしてもM4全体をcompletedにせず、M4-8の外部環境証拠とM4-9 final auditを必須にする。
- 観察（M4-7初回試行）: managed sandboxにはC compilerがなく、loopback socketも許可されなかった。また、live MT5 runnerとreal R2 credentialsがない。
  判断: Raceはroot不要の一時toolchainとloopback許可付き実行でLinux-equivalent evidenceを取得したが、real R2、read-only credential、24時間soakはskipやfake testで代替せず、外部gateの未完了証跡とoperator runbookをtracked artifactとして残す。
...
- Decision: disk criticalでもproof条件を緩和せず、prunable artifactがなければreadinessを落とし、new batchへdurable ACKを返さない。
  ...
  Date/Author: 2026-07-16 / Codex
- Decision: credential変更はGatewayを停止した市場休場中に手動で行い、実行中のGatewayで認証情報や公開担当を切り替える機能を提供しない。
  Rationale: 現行運用では同時writerを許容する必要がなく、複雑な切替機能を製品へ持ち込むより、停止・設定更新・再起動・接続確認の手順で十分だからである。
  Date/Author: 2026-07-17 / User/Codex
- Decision: external gateが未実施またはharness不在の場合は、実行手順を追加してもM4 acceptance checkboxをcheckしない。
  Rationale: safe-to-run documentationはreal R2、credential変更、実broker、forced rebootの観測事実を代替しないためである。
  Date/Author: 2026-07-16 / Codex
...
M4-7の終了時には、local correctnessとresource boundを証明しても、real broker、forced reboot、credential変更、real R2の外部証拠は未完了として残す。
...
`internal/r2`はpublisher claim、immutable raw/replay publication、bounded observation、receipt、scope/epoch lockを所有する。
...
M4のtrust boundaryでは、caller supplied path、SQLite stage、publication receiptの存在だけ、古いremote observation、remote list順序、wall clock単独、disk pressure単独を削除authorityにしない。
...
M4の実装対象はproof-gated local WAL/raw-outbox pruning、disk pressure、scope分離した複数producer運用、read-only `tick-api`、運用health、fault injection、24時間以上の実機soakである。
...
能動的late-history discoveryは別scopeまたはfollow-upとし、M4 acceptanceへ暗黙に含めない。
...
M4-1で`MaxPruneCandidates`、`MaxProofObjects`、`MaxProofBytes`、`MaxManifestNodes`、`MaxAPIRequestBytes`、`MaxAPIResponseItems`、`MaxConcurrentRequests`、`RequestTimeout`を設定とcontractへ固定する。
...
Raw segmentは、byte-identical remote raw objectがExactで、全accepted batchがbranchのない一意なhighest valid raw-day manifest集合から発見可能で、独立day/scope verification reportが一致する場合だけproof候補になる。
...
### M4-4 credential change boundary

M4-4はruntime機能を追加せず、credential変更を市場休場中に行う運用境界だけを文書化する。
Gatewayを停止し、WAL、journal、raw outboxのdurable状態を確認してからcredential bundleを更新し、
Gatewayを再起動する。再起動後にstatus、R2接続、既存objectの読み書きを確認する。
実行中のGatewayで認証情報や公開担当を切り替える機能、オンライン切替artifactは提供しない。
...
各scopeは一つのprovider、stable feed、exact source symbol、scope、publisher epoch、listen address、WAL root、journal path、outbox root、lock root、receipt rootを持つ。
...
Endpointは次を提供する。
    ...
    GET /v1/datasets
    GET /v1/datasets/{dataset}/scopes
...
Raw/replay listはdataset、scope、date、replay contract、conversionをtyped queryとして検証し、bounded paginationとstable orderを使う。
...
API processは`r2.ReadBackend`相当のread-only capabilityだけを構築し、write backend、publisher、retention executorをimportしない。
...
M4-7ではfake producer、fake read backend、fake conditional backend、fake clock、fake filesystem usageを使うend-to-endを追加する。
...
Fault matrixはprocess crash、partial WAL、checkpoint crash、delete failure、clock regression、remote branch、manifest gap、publisher conflict、R2 outage、R2 SDK timeout、disk high-water、HTTP timeoutを含む。
...
M4-8は外部環境gateであり、隔離bucketまたは`smoke/` immutable root、read/write credential、別read-only credential、Windows/MT5、実broker接続、forced rebootを許可できるhostを要求する。
必要なcredentialまたは明示確認がない場合はskip理由を記録できるが、M4全体は未完了のままにする。

Real R2 smokeはproduction `v1/`を使わず、synthetic dataset専用scopeでimmutable raw/replay publication、read-only fetch、same-content retry、different-content rejection、timeout recoveryを確認する。
...
注入eventはGo process restart、MQL Service restart、MT5 terminal restart、forced OS reboot、network/R2停止、R2 SDK timeout/retry、disk high-water、Gateway長時間停止を含む。
各eventは開始時刻、終了時刻、期待状態、observed status、operator action、recovery rootを記録する。

Soak後は別hostまたは少なくとも空の別cacheとread-only credentialだけを使い、raw-day manifest、全参照sealed WAL object、scope chain、replay manifest、part manifest、Parquet schema/hash/row-chain、API fetch planを検証する。
...
M4-9では`docs/operations/`へday operation、disk pressure、pruning、restore、credential変更、R2 outage、forced reboot、soakのrunbookを追加する。
...
M4-4ではcredential変更runbookの市場休場中手順を確認する。オンライン切替専用testやbuild tagは持たない。
...
R2 smokeは`r2_smoke` build tagのraw smokeだけを実行する。
environment variable名とcommandは`docs/operations/r2-smoke.md`へ固定し、secret valueを
...
M4-1の受入条件は、retention local identity、HTTP semantics、resource limits、negative classificationが文書、Go、Pythonまたはcontract testで一致することである。
...
M4-4の受入条件は、市場休場中のGateway停止、credential設定更新、再起動、status/R2接続確認の手順がrunbookへ記載され、稼働中のwriter切替機能を持たないことである。
...
M4-8の受入条件は、隔離real R2 raw smokeと24時間以上のlive MT5 soakがpassし、別cache/read-only credential verificationが成功することである。
...
実Tick、WAL、SQLite、Parquet、R2 object、credential、token IDの原文、host固有absolute pathをcommitしない。
...
external R2/MT5 runbook、M4 external gate record、secret-free external evidence template、final audit checklistを追加した。real R2、read-only credential、
24時間soakが未実施で、Race raw artifactのdurable retentionもfinal audit待ちのため、delivery statusはincompleteのまま維持する。

2026-07-17 R2 smoke config update: smoke用環境変数からM2/REAL/SMOKE confirmationの名前を削除し、
`TICK_R2_BUCKET`、`TICK_R2_IMMUTABLE_ROOT`、`TICK_R2_ENDPOINT`、`TICK_R2_ACCESS_KEY_ID`、`TICK_R2_SECRET_ACCESS_KEY`へ統一した。
`TICK_R2_IMMUTABLE_ROOT`は`smoke`または`smoke/`で始まる隔離namespaceを要求し、実行選択は`r2_smoke` build tagに限定する。
repository-local ignored `env.local`のkeyとroot shapeを確認後、R2 smokeを実行したが、サンドボックス外で`remote permission denied`となった。
これはR2 credentialまたはbucket scopeの修正待ちであり、M4-8は引き続きincompleteである。

2026-07-17 R2 smoke retry update: bucket設定修正後、同じ`env.local`境界で`go test -tags r2_smoke ./internal/delivery -run TestR2Smoke -count=1 -json`を再実行し、
`2026-07-17T15:42:03+09:00`開始、`2026-07-17T15:42:08+09:00`終了でpassした。
credential value、endpoint、bucket名、object keyは記録していない。
raw smokeはpassしたが、別cache/read-only credential verification、MT5 24時間soak、final auditが未完了のためM4全体はincompleteのままである。

2026-07-17 R2 smoke layout update: smoke testのR2階層から余計な二重run rootを削除し、
`TICK_R2_IMMUTABLE_ROOT=smoke`などの上位rootの下に`gateway=<gateway-id>/run=<UTC-based-run-id>`を作るようにした。
また、`m2-e2e-*`の長いscopeを実R2 smokeへ流用せず、v1のR2物理階層をtick専用の`source=<source>/symbol=<symbol>`へ簡略化する方針にした。
将来、live以外の系列分割が必要になった場合はv1 keyへscope/seriesを足さず、v2 layoutへ移行する。
Gateway IDはsmokeの実行主体を区別する固定階層であり、run IDはUTC時刻ベースで毎回一意にする。
この階層へ変更後の現在コードで実R2 smokeを再実行したが、`2026-07-17T16:11:38+09:00`に`remote permission denied`で失敗した。
