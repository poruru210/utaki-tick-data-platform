...
M2R-3は、read-only ArchiveReader、tickctlのraw commands、tick-verify day/scopeを実装します。
...
Parquet、replay-day manifest、part manifest、運用時の認証情報変更、pruning、Worker、HTTP service、live broker collectionはM2の対象外です。

## Progress

- [x] 2026-07-15にbranch agent/m2-raw-offhost-deliveryがcommit 510c014でM2R-1初回実装を完了しました。
- [x] 510c014でScopeConfig、archive-config hash、scope descriptor、raw_set_root、revision、canonical JSON、Go/Python fixture verifierを追加しました。
- [x] 510c014でcross-day batch、same-object disjoint range、zero-record sentinel、CopyTicksError、scope continuityの初期contractを追加しました。
...
- [x] 2026-07-15にM2R-2のrclone lock、scope layout、conditional publisher claim、local exclusive lock、独立SQLite journal、revision graph validator、出版reconcileを実装しました。
...
- [x] post-review correctionとして、R2 scope prefixへdataset identityを追加し、同じprovider/feed/symbol/scopeを持つ別datasetのnamespace衝突を防止するfocused testを追加しました。
...
- [x] 2026-07-15にM2R-3のread-only ArchiveReaderV1、tickctl raw commands、tick-verify day/scopeの実装を開始しました。
- [x] M2R-3でread-only backend分離、strict reader config、scope discovery、date単位snapshot graph、streaming cache、WAL restoration、day/scope report、CLI testsを実装しました。
- [x] M2R-3 focused testでempty cacheのexact-key/digest fetch、zero-record error batch、remote mutation、corrupt cache、stream failure、revision branch、day overclaim防止、scope root検証を確認しました。
- [x] M2R-3のCLI testでdatasets、scopes、snapshots raw、fetch、tick-verify day、tick-verify scopeのstable JSONとnonzero error境界を確認しました。
- [x] local環境ではgccとclangが存在しないためrace testは実行せず、review修正後のWindows CI push run `29380482973`とPR run `29380484762`でrace検証を成功させました。
- [x] M2R-3のArchiveReader、tickctl raw commands、tick-verify day/scopeの実装を完了しました。
...
- A/B/A scopeではday AのobjectsがAとCだけを参照し、middle Bをchain_objectsだけへ保持することで、raw_set_rootとchain continuityの責務を分離できます。
...
- R2 prefixがprovider/feed/symbol/scopeだけを含むと、ScopeConfigのDatasetIDが異なるscopeが同じimmutable namespaceへ衝突するため、datasetをprefixへ含めます。
...
- ArchiveReaderのrevision graphは複数UTC dayのmanifestを一括比較できないため、immutable day manifestのdateごとに完全なrevision graphを検証し、その後scopeのsegment集合を統合します。
- read-only fetchではS3 remote keyをscope-relative canonical keyから毎回Layoutで導出し、local cacheとconsumer outputは検証済みdigest basenameだけを使います。
...
- R2 smokeをdefault testへ含めるとcredentialとnetworkが通常gateへ混入するため、build tag、isolated `smoke/` immutable root、synthetic bytesを同時に要求します。

## Decision Log

- 決定: raw WAL object keyをscope-relativeなobjects/raw/wal-<64 lowercase sha256>.rtwへ固定します。
...
- 決定: ImmutableRootは設定値として一度だけscope prefixの上位へ付け、RcloneRootは同じcanonical relative keyから独立にlocatorを導出します。
- 理由: S3 keyとrclone locatorのどちらにもv1やscope prefixを二重連結せず、raw objectとmanifestのrelative keyを不変に保つためです。
...
- 決定: tick-verify dayはanchored day slice、tick-verify scopeはscope genesis to rootと明示し、day reportはgenesis verifiedをfalseに固定します。
- 理由: day manifestのanchor検証とscope全体のgenesis検証を同じ成功表示へ混ぜないためです。
- 決定: ArchiveReaderのremote manifest graphはUTC dateごとにValidateRevisionGraphを実行し、scope verifyでは全dateのChainObjectsをhashとsequence boundsでdeduplicateします。
- 理由: revision predecessorは同一dayのrevisionに限定され、異なるdayのrevisionを一つのbranchへ誤結合してはいけないためです。
- 決定: reader cacheとfetch outputのraw basenameはSHA-256から導出し、remote scope-relative keyはverified layoutで独立に再構成します。
- 理由: remote path traversalとscope prefixの二重連結を排除し、empty cacheからの復元経路を単純に検証できるためです。
- 決定: day verificationはarchive.VerifyRawDaySnapshotを実行してもgenesis_verified=falseとanchored_day_sliceを返し、scope verificationだけがzero genesisからthrough-rootを主張します。
- 理由: day sliceの前方anchorとscope全体のgenesis証明を明確に分離するためです。
...
- 決定: R2 smokeは`r2_smoke` build tagと`smoke/` immutable rootを満たす場合だけ実行し、remote objectをdelete、move、sync、overwriteしません。
...
- 決定: Parquet、replay-day、part manifest、運用時の認証情報変更、pruning、Worker、HTTP、live brokerはM2から除外します。
- 理由: raw evidence deliveryとderivative replay、運用時の認証情報管理、service runtimeの責務を混ぜないためです。

## Outcomes & Retrospective

510c014のM2R-1初回実装で、Protocol V1のcanonical JSON、raw_set_root、ScopeConfig、scope descriptor、raw-day buildとstrict decodeの基礎が完成しました。
...
M2R-4のfake end-to-end testはverified sealed WALからraw-day manifest、fake publication、empty-cache fetch、BatchFrameV1復元、zero-record error batch、anchored day report、scope genesis reportまでを一つのnetwork-free testで確認します。
...
M2R-3ではtickctlとtick-verifyのJSON runnerを追加し、day reportのgenesis overclaimを防ぎ、scope reportのzero genesisからrequested rootまでの証明を分離しました。
...
実R2 smokeはisolated bucket、`smoke/` immutable root、credentialが同時に存在する実行環境を提供されていないため実施せず、remote mutationの実環境確認はM2R-4のoptional smokeへ残します。
...
internal/archiveはverified RawObject、ScopeConfig、scope descriptor、raw-day manifest、local semantic verifierを所有します。
...
ScopeConfigはdataset_id、provider_id、stable_feed_id、exact_source_symbol、broker_server_fingerprint、gateway build identity、producer build identity、day_definition_id、settle_policy、publisher_id、publisher_epoch、Protocol limitsを持ちます。
...
EnsureScopeDescriptorはhash-derived safe pathへno-clobber descriptorを作り、same content retryを成功させ、different contentをarchive.ErrIntegrityにします。
...
scope prefixはhash-safe scope identityから導出し、configured ImmutableRootを一度だけ上位へ付け、S3 keyとrclone locatorは同じscope-relative keyから独立に導出します。
...
publisher claimはscopeとpublisher epochをbindし、conditional createとtransitionをno-clobberで検証します。

local lockは同じscopeとpublisher epochの同時publisherを一つに制限し、stale lock recoveryをowner identityとlease evidenceへ限定します。
...
ArchiveReaderはread-only tokenとremote manifestからscope、scope、revision、objects、chain_objectsを解決します。
...
tickctl raw commandsはdatasets、scopes、snapshots、fetchのread-only raw delivery contractを提供します。

tick-verify dayはmanifestのselected chain sliceとday rangesを検証し、scope genesisからの完全chain検証とは主張しません。

tick-verify scopeは指定scopeのchain object setをsequenceとhash continuityに沿って検証します。
...
M2R-3ではArchiveReaderとtickctl raw commandsをread-only contractへ接続し、dayとscopeのverification resultを分離します。
...
BuildRawDayManifestはverified sealed WAL以外、caller-forged key、discontinuous scope chainを受け付けません。
...
M2R-3のreader testはread-only fake backend、empty cache、exact key、digest selector、remote mutation、stream failure、revision graph、day report、scope rootを確認します。
...
M2R-4のfocused integration検証はmise exec -- go test ./internal/r2 ./internal/delivery ./cmd/tickctl ./cmd/tick-verify ./internal/archiveで実行し、fake publication、empty-cache fetch、BatchFrameV1復元、zero-record error、day report、scope reportを確認します。

M2R-4のoptional smoke compile検証はmise exec -- go test -tags r2_smoke ./internal/delivery -run TestR2Smoke -count=1で実行し、R2環境変数がない場合は明示理由付きskipとします。
...
scope descriptorは同一canonical config retryを成功させ、異なるconfigをno-clobberで拒否します。
...
M2R-4のoptional smoke artifactはinternal/delivery/r2_smoke_test.goであり、r2_smoke build tag、synthetic bytes、isolated `smoke/` immutable root、no-overwrite境界を持ちます。
...
EnsureScopeDescriptorはrootとScopeConfigを受け、no-clobber descriptor pathまたはerrorを返します。
...
operator configはWALだけでは証明できないdataset identity、scope identity、provider identity、exact source identityのtrust rootであり、その限界をdocsへ明記します。
