...
- [x] local環境ではgccとclangが存在しないためrace testは実行せず、Windows CIのpush run `29379100753`とPR run `29379114434`でrace検証を成功させました。
...
- [x] PR #3は`7ae38c2`をheadとしてopen、clean、mergeableであり、thread-aware review stateに未解決threadはありません。
...
M2R-4のreal-R2 smokeは必要なopt-in条件がlocal環境にないためskipし、GitHub Actionsのcheck push run `29379100767`、check PR run `29379114428`、Windows race push run `29379100753`、Windows race PR run `29379114434`は全て成功しました。
...
M2R-3のrace testはgccとclangがlocalに存在しないため実行せず、M2R-4のWindows race workflowで対象packageをまとめて実行し、push run `29379100753`とPR run `29379114434`を成功させました。
...
local環境にgccとclangがないためrace testは実施しませんでしたが、Windows race workflowのpush run `29379100753`とPR run `29379114434`で解消しました。
...
push run `29379100753`とPR run `29379114434`でこの検証を成功させました。
...
M2R-4のrace検証はmise exec -- go test -race ./internal/ingest ./internal/wal ./internal/archive ./internal/r2 ./internal/delivery ./internal/catalogをWindows CIで実行し、push run `29379100753`とPR run `29379114434`を成功させました。
