...
- [x] (2026-07-15、G1RC regression reclose) Empty terminal／predecessor、zero／mixed／unproven、terminal mismatch、trusted Layout adapter、pre-lock exact／aggregate／overflow／exhaustion、lock-not-acquired、claim ownershipを個別再検証した。
  ...
  Aggregate resource enforcementは全scopeで共有し、canonical final observationは最後のfresh pass counterをbindする判断へ固定した。
...
- [x] (2026-07-15、G5-R5) Thin publisher、canonical bundle／terminal observation receipt、no-clobber store、event非authority、全round shared budget、MaxRoundsを統合した。
  最初のremote observationからreceipt保存完了までscope lockを保持し、実行ごとにapproved ObjectID一件だけをexecutorへ渡してfresh reobserveする。
...
R3のfinal observationはclaim、raw、Parquet、part、part graph、replay manifest、revision graphがすべてExactで、scope／epoch lockをfinal digest生成直前にも再assertできた場合だけ作成できた。
...
最終観測はpoint-in-time evidenceであり、receiptがfuture mutation、admin-resistant WORM、failoverを証明しないことを明示しないと、receiptの意味を過大評価する。
...
- Decision: M3 replayはpublisher claimを作成せず、M2 raw publicationが作成した既存claimのExact observationだけを要求する。
  Rationale: claimはscopeのpublisher identityであり、Parquet derivativeの処理状態や分散mutexではないためである。
  Date/Author: 2026-07-15 / Codex
...
M2 raw publicationがpublisher claimをIf-None-Match:*で作成し、scopeとpublisher epochのlocal exclusive lockを使う。
...
そのまま保持する候補は、`internal/protocol`と`internal/archive`のpart／replay manifest verifier、`internal/r2/layout.go`のtrusted key導出、`internal/r2/tool.go`のpinned rcloneとcommand allow-list、`internal/r2/lock.go`のscope／publisher-epoch local lock、M2のpublisher claim作成、local no-clobber保存primitiveである。
...
publisher constructorは任意lock filenameを受け取らず、trusted Layoutのscope／publisher epoch scopeとcaller-supplied lock rootからcanonical `PublicationLockPath`を導出する。
...
R2 smokeは、既存の明示的なisolated credential、isolated bucket、`smoke/` immutable root、non-production scopeが全て存在する場合に限る。
...
この不変条件は、scopeとpublisher epochの一台一hostというM3 operational preconditionと、trusted bucket prefixが他writerから同時変更されない脅威モデルの下で成立する。
...
receiptはpoint-in-time evidenceであり、将来のremote mutation、administrator-resistant WORM、failover、consumer deliveryを保証しない。
...
発見事項は、`makeProtocolFinalObservation`が必要bytesまでcounterを自己補正して共有scope budgetを消費しないことと、listで得た`RemoteObject.Size`をParquet content検証へ結び付けていないことである。
...
publisher constructorはlock rootからtrusted Layoutのscope／publisher epoch pathを導出し、任意filenameをlock identityとして受け付けない。

revision 3 successor、4-part fresh-observation publication、round不足のlock前拒否、canonical scope／epoch lock pathをfocused Goで受入した。
