...
- [ ] internal/walへsealed trailer、segment verifier、rotation、startup recoveryを実装する。
- [ ] internal/archiveへbyte-exactなcontent-addressed outbox promoteを実装する。
- [ ] segmentをまたぐGatewayのjournal rebuildとduplicate判定をtestする。
- [ ] partial trailer、corruption、same-content retry、same-key different-contentをtestする。
- [ ] README、roadmap、親ExecPlan、検証記録を実装結果へ更新する。
- [ ] mise run checkとWindows Race Detectorを成功させる。
- [ ] コードレビューで指摘を評価し、妥当な指摘を修正する。
- [ ] commitとpushを行い、main向けPull RequestをReadyかつmergeableな状態へ整える。
...
- Decision: standalone verifierは最初のentryが持つprevious_entry_hashをChainStartとして返し、Storeのstartup scanが直前segmentのChainRootとの一致を検証する。
  Rationale: trailerはpredecessor anchorを持たないが、各entryはprevious_entry_hashを持つため、segment内部の検証とcampaign内の連結検証を分離できる。
  Date/Author: 2026-07-14 / Codex
...
完了時に、実現した動作、残したM2範囲、検証結果、今後の注意点を記録する。
