...
- [ ] G2 gateの最終証跡として、Linux／Windows CIのrace実行結果を取得する。ローカルはC compiler不在のため`go test -race`を実行できない。
- [ ] G3独立レビューの指摘を修正し、automatic revisionの時刻、revision predecessor gate、fake ticker、planner検証を完了する。
- [ ] G4 SDK uploader、全量verification、durable retryを常駐workerへ接続する。
- [ ] G4独立レビューの指摘を修正し、remote intent復元、local／remote state authority分離、境界再開テストを完了する。
- [ ] G5 crash recovery、backpressure、health、prune gateを接続する。
...
- `prune-local`へ`remote_verified`必要条件と`local_pruned`記録を追加する。
...
| production `run`はR2 configを要求しない | local WALだけで起動できる | `run`だけpublication configを必須にする | 旧configはmigrationなしで起動しない | fail-fast migration errorとexample更新 | uploadなしで正常起動するsilent fallback |
...
| config loaderはflat形式だけ | `[credentials]`等を読めない | existing TOML dependencyでtyped root configを追加する | strict decodeで旧曖昧入力が拒否され得る | `internal/config`を正本にし、既存top-level keyはtyped compatibility fieldで読む | 独自parserへsection処理を継ぎ足す |
...
raw outboxの削除成功後だけ、publication objectの同じlocal pathへ`local_pruned`をdurably記録する。
sealed WALの削除完了は既存prune checkpointで記録し、raw outboxが残っているのにpublication objectを`local_pruned`へ進めない。
...
既存top-level keyはtyped compatibility fieldとして同じ意味でdecodeし、独自parserへsection supportを追加しない。
...
- 旧credential configからFileProviderへのmigrationがrunbookに明記される。
