...
R2 publicationの正本は本計画とする。
AWS SDK for Go v2、条件付きPUT、全量Get検証、`remote_verified`状態を本計画で管理する。
...
- [x] (2026-07-16) G0レビュー後、明示的な`New...`＋`Start`／`Stop`境界、R2 JournalとCatalogのauthority、clock注入、config変換境界を確定した。未リリースのため、旧`Open`／`Close`／暗黙clockの互換wrapperは採用しない。
...
- [x] (2026-07-17) 未リリース前提を全体へ適用し、`ingest.Open`、`Gateway.ListenAndServe`／`Gateway.Close`、`ingest.LoadConfig`、`wal.Open`、`journal.Open`、`r2.OpenPublicationJournal`、各Store／PublicationJournalの旧`Close`を削除した。`Gateway`は`Start`／`Stop`だけ、`r2.NewPublisher`は明示clock必須の新signatureだけを公開する。one-shot commandとtestも`New...`後の明示的`Start`／`Stop`へ移行した。
- [x] (2026-07-17) R2の旧default credential chain／環境変数名指定constructor（`NewS3Backend`／`NewS3BackendWithEnv`）も削除し、real smokeを`FileProvider`＋`CredentialBackend`へ移行した。`CredentialBackend`は必要なbounded read capabilityだけを提供し、R2 API接続経路を新方式へ統一した。
- [x] (2026-07-17) 互換層削除後の`mise run check`、`go vet ./...`、`go build ./...`、`go test ./...`を実行し、全て成功した。これはlocal verificationであり、未push差分のGitHub CI証拠ではない。
- [x] (2026-07-17) 追加のR2 credential cleanup後に`mise run check`、`go vet ./...`、`go build ./...`、通常Go test、tag付きreal smoke compile-onlyを再実行して成功した。GPT-5.5 xhigh workerによる追加差分レビューもPASS（actionable findingsなし）となった。
- [x] (2026-07-17) 未使用のオンライン認証情報切替機能、専用API、fixture、smoke、Protocol verifier経路を削除し、credential変更を市場休場中の停止・設定更新・再起動に限定した。PublicationJournalのStop後再起動テストを追加し、GPT-5.5 xhigh workerの修正後レビューはPASS（actionable findingsなし）となった。
- [ ] G6 Linux／Windows CIとreal R2 smokeでend-to-end経路を検証する。

## Initial baseline (historical)

以下はM4実装前（2026-07-16）の状態であり、現在の実装を表すものではない。

### Pre-M4 entrypoints (historical)
...
### Pre-M4 local data path (historical)
...
### Pre-M4 remote data path (historical)
...
### Pre-M4 credential access (historical)
...
### Pre-M4 tests and CI (historical)
...
- 観察: Gateway config loaderはflatな独自`key=value` parserで、Provider仕様の`[credentials]` tableを読めない。
  ...
  判断: 新しいtyped sectionだけをstrict decodeし、旧top-level keyやcredential env fieldはunknown field errorとして拒否する。
...
- 観察: 現在の`Open`系APIはfile、SQLite、recoveryをconstructor内で実行し、初期化と所有権を呼び出し側から隠す。
  判断: `New...`は副作用のないconstructor、`Start`／`Stop`はresource ownerの唯一のライフサイクル契約とし、`Open`系の別経路は持たない。
- 観察: `internal/r2.PublicationJournal`はremote intent、object upload state、verification bindingを記録する。
  判断: これらはremote publicationの現行契約としてR2 Journalが所有し、local retry、pending列挙、manifest planning、`local_pruned`判定は`internal/publication.Catalog`だけを正本とする。
- 観察: `r2.Publisher`はremote verification時刻を必要とする。
  判断: Publisherは明示clockを受けるconstructorだけを持ち、productionは`time.Now`、testはfake clockを渡す。
...
- Decision: Fx importは`cmd/tick-gateway`と`internal/app`およびそのtestへ限定する。
  ...
  Date/Author: 2026-07-16 / Codex
- Decision: production Fx graphはWAL、ingest journal、publication catalog、R2 backend、Gateway、publication coordinatorを明示的なconstructorで組み立てる。
  Rationale: resource acquisitionをconstructor内へ隠す別経路を持たず、Fx graph validation、開始順序、部分起動rollbackを証明可能にするためである。
  Date/Author: 2026-07-16 / Codex
- Decision: `ingest.Open`、`wal.Open`、journal open API、`Gateway.Close`は削除し、one-shot commandは`app.LocalGatewayRuntime`、domain testは`New...`＋`Start`／`Stop`を使う。
  Rationale: 未リリース段階で旧呼出しを温存すると、初期化責務とresource所有権が二重化するためである。
  Date/Author: 2026-07-16 / Codex
...
- Decision: application configは`internal/config`のstrict typed TOML loaderを正本とし、既存のGo TOML依存を再利用する。
  ...
  Date/Author: 2026-07-16 / Codex
- Decision: `ingest.Config`はdomain runtime valueとして維持し、`internal/config.Config`からの明示的な変換を一箇所へ置く。
  Rationale: production composition rootとone-shot commandがstrict application configを直接domainへ持ち込まず、変換境界を一つに固定するためである。
  Date/Author: 2026-07-16 / Codex
- Decision: R2 Journalはremote intent、object transition、ETag、verification timeだけを所有し、常駐runtimeのlocal queue、retry、prune authorityは持たない。
  Rationale: remote API境界の状態とlocal orchestrationの状態を一つのschemaへ戻さず、責務を一意にするためである。
  Date/Author: 2026-07-16 / Codex
...
- Decision: `local_pruned`は`internal/publication.Catalog`だけが所有し、R2 PublicationJournalは`remote_verified`までを所有する。
  Rationale: local削除の完了はretention executorとlocal pathの事実であり、remote intent／ETag／verificationを所有するR2境界へ書き戻すとlocal lifecycleが二重化するためである。
  Date/Author: 2026-07-17 / Codex
...
- `internal/config`へstrict typed application configを追加し、旧flat top-level keyはdecodeしない。
...
| credentialを環境変数から読む | reader／retention／backendがsecret envを参照する | FileProviderへ置換する | deployment設定の変更が必要 | 新しいfile bundleを必須にする | production EnvProviderを残す |
...
| `ingest.Open`がresourceを開く | WAL、journal、disk、recoveryが一つの関数に隠れる | 各resourceへconstructorとStart／Stopを追加する | one-shot commandとtestの呼出しが変わる | `Open`を削除し、production graphは明示constructor、one-shotは`app.LocalGatewayRuntime`を使う | Fx OnStartから旧`ingest.Open`を呼ぶadapter |
| `Close`がhandlerを無期限に待つ | Stop deadlineを守れない | bounded `Stop(ctx)`を唯一の停止APIにする | timeout時のerrorが増える | deadlineを伝播し、resource ownerがWAL／journalを逆順に停止する | 無期限waitまたは`Close`互換wrapper |
...
旧fieldを指定した場合はsilent fallbackせず、新しいFileProvider設定を示すsecret-free errorを返す。
...
`ingest.Open`、`wal.Open`、既存journal open helperは存在せず、このproduction graphにも含めない。
one-shot commandだけが`app.LocalGatewayRuntime`で同じconstructorとStart／Stopを明示的に合成する。別の初期化規則は持たない。
...
8. remote PublicationJournal、local Publication Catalog、ingest journal、WALを逆順で`Stop`する
```
...
シニアreviewではPR規模、契約境界、test分割、置換範囲、運用診断性を確認する。
...
- R2 Journalはremote intent／verificationだけを所有し、local retry／manifest／prune authorityへ流用しない。
...
### G1: Credential Provider and config contract
...
`internal/config.Config`から`ingest.Config`とone-shot command用valueへ変換する関数を一つだけ設け、production `run`は変換済みvalueをFxへ供給する。
`ingest.LoadConfig`や旧flat parserは追加せず、application config loaderを唯一の設定入口にする。
...
`NewPublisher`は明示clockを必須引数とし、暗黙clockのoverloadは追加しない。
write backendは`CredentialBackend`だけをcomposition rootへ公開し、AWS default credential chainや環境変数名指定の`NewS3Backend*` constructorは持たない。
...
`ingest.Gateway`はbounded `Stop(ctx)`だけを公開し、WALとjournalはcomposition ownerが別々に停止する。
Gateway constructorへWAL、ingest journal、disk pressure decisionを明示的に渡す。
Gateway OnStartはlistenerを同期bindしてからaccept goroutineを開始する。
bind失敗または後続Start失敗では、開始済みresourceを逆順に`Stop`する。
one-shot commandは`app.LocalGatewayRuntime`を使い、constructor、Start、Stopの所有権を明示する。
...
最大のoperational riskは、設定契約が一つに固定されていないことである。
silent local-only fallbackや移行aliasは採用せず、新example、runbook、doctor相当のconfig validationを提供する。
...
本計画は未リリースのため、旧binaryへの互換rollbackや旧schemaからの自動migrationは提供しない。
schema変更後は修正版をforward deployし、local disk、WAL、publication stateを削除せずに再検証する。
