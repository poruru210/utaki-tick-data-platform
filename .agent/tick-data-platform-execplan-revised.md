...
M2のローカル基盤では、WAL sealとrotation、segmentをまたぐjournal rebuild、sealed verifier、byte-exactなcontent-addressed outbox promoteを実装した。
R2 publication、raw-day manifest、read-only fetchとday/campaign verifierは未実装であるため、M2全体は完了していない。
...
改訂記録（2026-07-15）: M1のPull Request #1 mergeと、M2のローカルWAL seal、segment recovery、sealed verifier、content-addressed outbox promoteを反映した。M2のR2 publicationとraw deliveryは未完了として残した。
