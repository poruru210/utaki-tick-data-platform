...
- [x] (2026-07-16) M3でordered overlap reducer、version-scoped Parquet、day-local part manifest、replay-day snapshotとdelivery contractを実装し、whole-M3 final re-auditをpassした。詳細な実装記録は`execplan/2026-07-15-m3-replay-parquet-delivery.md`に保持する。
...
- [x] (2026-07-16、M3 merge) M3実装をPull Request #4のmerge commit `cb72752a651c88c3027b409f6f205ac9236f28b8`としてmainへ反映した。
- [x] (2026-07-16、M4-0 plan freeze) M4の実装順、停止条件、外部環境gateを`execplan/2026-07-16-m4-production-operations-http-delivery.md`へ切り出した。M4実装はまだ開始していない。
...
M3はordered overlap reducer、Parquet、part／replay manifest、immutable replay publication、read-only replay delivery、production M2 publisherからのnetwork-free end-to-endを実装し、whole-M3 final re-auditをpassした。
...
M3実装はPull Request #4のmerge commit `cb72752a651c88c3027b409f6f205ac9236f28b8`としてmainへ統合済みである。
...
M4の詳細な実施計画と検証手順は`execplan/2026-07-16-m4-production-operations-http-delivery.md`へ記録する。
...
operator向けcommandは次のとおりとする。
    ...
    go run ./cmd/tick-gateway prune-local --config local/tick-gateway.toml --retention-config local/tick-retention.toml --dry-run
...
改訂記録（2026-07-16 M3-CLOSEOUT-M4-PLAN）: Progressの旧M3未完了checkboxをwhole-M3 final re-auditとPull Request #4のmerge結果へ同期した。

M4の正本となる専用ExecPlanを`execplan/2026-07-16-m4-production-operations-http-delivery.md`へ追加した。

M4-0は文書計画だけを固定し、pruning、handover、remote write、HTTP service、実機soakを開始または完了したとは扱わない。
